...
/* 로고: 변형문제와 동일한 @page 푸터(running element) 방식.
   인쇄 시 페이지마다 우하단 여백에 표시, 화면(열기)에선 떠다니지 않게 숨김. */
.footer-logo img{height:22mm;}
@media screen{ .footer-logo{ display:none; } }
...
def render_seosul_set(s: dict, teacher: bool = False,
    ...
    body = [f'''<div class="footer-logo"><img src="{_logo_uri()}" alt="로고"></div>
<div class="hdr">
  ...
  <span class="hdr-right">{ref_label}</span>
    ...
    for it in s["items"]:
        ...
        if it["type"] in ("SA",):
            ...
            body.append(head + cond +
                        f'<div class="bogi"><span class="bogi-label">보기</span> {bogi}</div>' +
                        rows + '</div>')
        elif it["type"] == "SC":
            bogi = ' / '.join(it["bogi"])
            summ = it["summary"]
            ...
            rows = "".join(f'<div class="wr-row"><span class="wtag">({k})</span><span class="wline"></span></div>'
                           for k in it["answers"])
            body.append(head +
                        f'<div class="summary-box">{summ}</div>' +
                        f'<div class="bogi"><span class="bogi-label">보기</span> {bogi}</div>' +
                        rows + '</div>')
        elif it["type"] == "SD":
    ...
    # ---- 정답 페이지 ----
    body.append('<div class="ans-start"></div>')
    body.append(f'<div class="ans-hdr"><span class="hdr-title">정답 및 해설</span>'
    ...
    body.append(f'<div class="ans-end"></div>')

    return (f'<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8">'
            f'<title>서술형 종합문제 - {school_name}</title>'
            f'<style>{_style()}{_EXTRA}</style></head><body>{"".join(body)}</body></html>')
