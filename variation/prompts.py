...
def build_summary_sentence_prompt(passage_text: str) -> str:
    return (
        ...
        "    도당고3 Latour 문항이 30단어다. 그 정도 분량이어야 (A)(B) 두 자리를 각각\n"
        "    3~9단어로 잡고도 뼈대가 남는다. 15단어짜리 문장은 빈칸 둘을 넣으면 뼈대가 사라진다.\n"
        "- ★ Build the sentence around ONE logical connective so it has two ends to blank:\n"
        "    so / because / whereas / while / making / although / by contrast.\n"
        "    'Depression has diverse causes, SO a single solution is impossible.'\n"
        "    'Studios connect artworks to context WHEREAS exhibitions isolate them.'\n"
        "  A flat sentence with no connective gives nowhere meaningful to blank.\n"
        "- ★ Also give the Korean translation of THAT SAME summary sentence (full_summary_kr). "
        "It must be a translation of the sentence you just wrote — not of the passage, not of anything else. "
        "PLAIN declarative style (~한다/~이다), never honorific (~합니다).\n\n"

        "## 그다음, 그 문장 안에서 빈칸 두 자리를 지목하라\n"
        "Students get the summary with (A)(B) blanked out, plus a shuffled word bank, and must\n"
        "rebuild your exact wording. So you must point at a stretch of the sentence you just wrote —\n"
        "★ do NOT retype it. Give the first and last words and the code will cut the span out.\n"
        ...
        "  ★ Retyping invites paraphrase. Even one changed word makes the cut fail.\n\n"

        "### 어디를 뚫을 것인가 — 학교 기출 실측 (26-1 기말 4문항)\n"
        "★ 빈칸 둘은 요약문의 '논리 관계 양끝'을 담는다. 뼈대(연결어)는 남기고 양끝을 뚫는다.\n"
        "  실제 출제된 네 가지 뼈대:\n"
        "    (1) 원인 → 귀결\n"
        "        'Depression (A), so the complexity of (B).'\n"
        "          (A) has diverse causes and symptoms          — 무엇이 사실인가\n"
        "          (B) the condition makes a single solution impossible to find — 그래서 무엇이 따르는가\n"
        "    (2) 기원 → 결과\n"
        "        'scientific facts emerge (A) of observations ..., making (B) extremely difficult.'\n"
        "          (A) a disordered array          — 무엇으로부터 나오는가\n"
        "          (B) assigning credit and dates  — 그래서 무엇이 어려운가\n"
        "    (3) 대비 — whereas / while / by contrast\n"
        "        'Studios and homes (A) whereas (B) that context.'\n"
        "          (A) connect artworks to their context        — 한쪽이 하는 일\n"
        "          (B) exhibitions isolate them in the absence of — 반대쪽이 하는 일\n"
        "    (4) 양보 → 조건\n"
        "        'Although X is essential, the stage work can be best appreciated (A) so as to ...'\n"
        "          (A) with the audience kept from noticing their presence — 어떤 상태일 때 가능한가\n\n"
        "  ★ 위 넷 중 지문 논리에 맞는 뼈대를 고르고, 그 양끝을 (A)(B)로 뚫어라.\n\n"

        "### 빈칸 자리를 정하는 순서 (이대로 하라)\n"
        "  1. 요약문을 쓴 뒤, 그 안의 연결어(so/whereas/making/because/although)를 찾아 표시한다.\n"
        "  2. 연결어 앞부분에서 '전제·원인·한쪽'을 담는 구를 (A)로 잡는다.\n"
        "  3. 연결어 뒷부분에서 '귀결·결과·반대쪽'을 담는 구를 (B)로 잡는다.\n"
        "  4. ★ 연결어 자체와 그 앞뒤 구두점은 빈칸 밖에 남긴다.\n"
        "     그것이 학생에게 두 빈칸의 관계를 알려주는 유일한 단서다.\n"
        "  5. 두 빈칸을 지우고 읽어보라. 남은 뼈대만으로 '무엇과 무엇이 어떤 관계인지'가\n"
        "     드러나야 한다. 안 드러나면 빈칸을 좁혀라.\n\n"

        "### 기출 실제 정답 — 이 정도 크기·성격으로 잡아라\n"
        "  뼈대: 'Depression (A), so the complexity of (B).'\n"
        "    (A) has diverse causes and symptoms                          [5단어]\n"
        "    (B) the condition makes a single solution impossible to find [9단어]\n"
        "  뼈대: 'scientific facts emerge (A) of observations ..., making (B) extremely difficult.'\n"
        "    (A) a disordered array                                       [3단어]\n"
        "    (B) assigning credit and dates                               [4단어]\n"
        "  뼈대: 'Studios and homes (A) whereas (B) that context.'\n"
        ...
        "  [X] 연결어를 빈칸 안에 넣기 — 'symptoms, so a single' 처럼 so 를 물면\n"
        "      학생이 두 빈칸의 관계를 알 단서가 사라진다\n"
        "  [X] 빈칸이 문장 대부분을 차지 — 뼈대가 없으면 문맥으로 풀 수 없다\n\n"

        "### 조건 — 어기면 그 항목이 버려진다\n"
        "  · ★ 각 span 은 3~9 단어. 학교 기출 실측 3~9, 평균 5.9 (7개 빈칸).\n"
        "    starts_with 부터 ends_with 까지 직접 세어라.\n"
        "    단어 수는 결과이지 목표가 아니다 — 논리 축이 3단어로 끊기면 3단어가 맞다.\n"
        "    다만 5단어 이상이 기출의 다수(7개 중 5개)다. 너무 짧게 자르지 마라.\n"
        "  · ★ blank_A 가 blank_B 보다 문장에서 먼저 와야 하고, 둘 사이에 최소 3단어를 두어라.\n"
        "    붙어 있으면 사실상 빈칸 하나가 된다.\n"
        ...
        "  · starts_with 가 문장 안에서 한 번만 나오는 표현이어야 한다. 'the' 'to' 같이 여러 번\n"
        "    나오는 말로 시작하면 자를 위치가 모호해진다.\n\n"

        "### 출력 직전 확인\n"
        "  1. starts_with / ends_with 가 full_summary 안에 글자 그대로 있는가?\n"
        "  2. 각 span 이 4~8 단어인가?\n"
        "  3. (A)가 (B)보다 앞이고, 사이에 3단어 이상 있는가?\n"
        "  4. 두 span 을 빼고 읽었을 때 남는 뼈대가 문장으로 성립하는가?\n\n"
